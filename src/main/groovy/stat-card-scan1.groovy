...
def rowCard = db.rows('''
	select count(id) as num, DATE_FORMAT(dd,'%m-%d') as dd from ih_wx_card group by DATE_FORMAT(dd,'%m-%d') order by DATE_FORMAT(dd,'%m-%d') desc limit 5;
'''
)
def rowCardFill = db.rows('''
	select count(id) as num, DATE_FORMAT(dd,'%m-%d') as dd  from ih_wx_card where name is not null group by DATE_FORMAT(dd,'%m-%d') order by DATE_FORMAT(dd,'%m-%d') desc;
'''
)

def rowExchange = db.rows('''
	select count(id) as num, DATE_FORMAT(dd,'%m-%d') as dd  from ih_wx_card_tpl_send_log group by DATE_FORMAT(dd,'%m-%d') order by DATE_FORMAT(dd,'%m-%d') desc;
'''
)
